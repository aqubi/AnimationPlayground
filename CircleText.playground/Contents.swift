...
arc1.path = UIBezierPath(arcCenter: arcCenter, radius: radius, startAngle: CGFloat(M_PI + diff), endAngle: CGFloat(-diff), clockwise: true).CGPath
...
arc2.frame = circleRect
arc2.path = UIBezierPath(arcCenter: arcCenter, radius: radius, startAngle: CGFloat(M_PI - diff), endAngle: CGFloat(diff), clockwise: false).CGPath
...
rootLayer.addAnimation(ani1, forKey:nil)
